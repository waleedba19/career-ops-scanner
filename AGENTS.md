...
## Groq AI Scoring
- **Model**: `llama-3.3-70b-versatile` (via Groq cloud API, `GROQ_MODEL` env)
- **API Key**: `GROQ_API_KEY` env var (set in GitHub Secrets)
...
- **Cloud-based**: No local server needed. Fast inference via Groq's cloud infrastructure.
- **Graceful fallback**: If GROQ_API_KEY is not set, keyword scoring continues as the primary gate.
...
groq_analyzer.py    — AI job scoring (Groq cloud)
...
- If Groq AI fails → keyword scoring continues as fallback
